...
/// Card indices whose other version is kept rather than reasoned about.
///
/// A camera's own index describes *this card*: it differs between two slots by
/// construction, it has no published structure to prove anything about, and it
/// is regenerated by the camera after a format. sluice cannot show its
/// difference is harmless, so it does not pretend to — it copies the other
/// card's version instead, which makes "erasing loses nothing" true rather than
/// assumed.
///
/// **Exact relative paths, never a size or a pattern.** A size-based version of
/// this rule was written first and failed its own safety test: a 26-byte `.ARW`
/// with a flipped bit was preserved and dropped from the failures, and a real
/// ARW at 25–80 MB would have been too. Matching the whole path means the rule
/// cannot reach a photograph, a clip, or anything else that is somebody's work.
/// Widen it only on evidence, one observed filename at a time.
const CARD_INDEX_PATHS: &[&str] = &["PRIVATE/DATABASE/DATABASE.BIN"];

/// A sanity bound, not the rule. The rule is the path above; this only stops a
/// pathological file claiming that name and doubling a session.
const MAX_CARD_INDEX: u64 = 256 << 20;

/// Whether this is a camera index whose second version is kept.
pub fn is_card_index(rel: &str, size: u64) -> bool {
    size <= MAX_CARD_INDEX && CARD_INDEX_PATHS.iter().any(|p| rel.eq_ignore_ascii_case(p))
}

/// Where the other card's version of an index is kept inside the session.
///
/// A reserved prefix a camera will not produce, and a real manifest entry rather
/// than a sidecar: it is the reason erasing that card loses nothing, so
/// `verify --drive` must re-check it like anything else.
pub fn kept_rel(rel: &str, card: &str) -> String {
    format!("_twin/{card}/{rel}")
}

/// Copy one card's version of an index into a destination and prove it landed.
///
/// Returns the hash read back **off the destination**, unbuffered, so the caller
/// can require it to equal what the card held. Nothing here trusts the write: a
/// copy that cannot be read back as the card's bytes is not a copy.
pub fn keep(src: &Path, dest: &Path) -> Result<(u64, u64)> {
    ...
    drop(out);
    let (hash, len) = super::unbuffered::hash_unbuffered(dest)?;
    if len != written {
        anyhow::bail!("{} is {len} bytes after writing {written}", dest.display());
    }
    Ok((hash, written))
}
...
mod tests {
  ...
  <Duration value="372"/>
    ...
    /// The rule is the whole path. This is the guard that keeps a card index
    /// from becoming "small files may differ", which is the version that would
    /// have masked a flipped bit in a photograph.
    #[test]
    fn only_the_named_index_is_kept_never_a_photograph() {
        assert!(is_card_index("PRIVATE/DATABASE/DATABASE.BIN", 9_670_656));
        assert!(
            is_card_index("private/database/database.bin", 1024),
            "exFAT is case-insensitive and so is this"
        );

        for not_an_index in [
            "DCIM/100MSDCF/DSC00001.ARW",
            "PRIVATE/M4ROOT/CLIP/C0010.MP4",
            "DATABASE.BIN",
            "PRIVATE/DATABASE/DATABASE.BIN.bak",
            "DCIM/DATABASE.BIN",
            "PRIVATE/DATABASE/OTHER.BIN",
        ] {
            assert!(
                !is_card_index(not_an_index, 26),
                "{not_an_index} is not the card index, whatever its size"
            );
        }
        assert!(
            !is_card_index("PRIVATE/DATABASE/DATABASE.BIN", 512 << 20),
            "a half-gigabyte file claiming that name is not an index"
        );
    }

    #[test]
    fn a_kept_twin_lives_under_a_reserved_prefix() {
        assert_eq!(
            kept_rel("PRIVATE/DATABASE/DATABASE.BIN", "C2"),
            "_twin/C2/PRIVATE/DATABASE/DATABASE.BIN"
        );
    }
